...
1. **Tier 1 (Major Factors, ~82% dynamic share)**: Bragg spot count ($N_B$) and Average Bragg SNR ($\text{SNR}$) co-equally drive the score.
...
4. **Bragg-Isolated SNR Calculation**:
   Average Signal-to-Noise Ratio ($\text{SNR}$) in the logit formula is computed strictly over confirmed Bragg reflections ($N_B$) in regularity analysis rather than all candidate spots ($N_{\text{spots}}$). This guarantees that low-significance noise blobs near the spot-finding threshold do not dilute the crystal's genuine reflection SNR or artificially depress the score.
5. **Post-Logistic Ice Penalty ($P_{\text{ice}} \in [0.0, 10.0]$)**:
...
- Background noise spots near detection threshold do not dilute reflection SNR for well-diffracting crystals.
